...
def draw_box_plot():
    ...
    df_box['month_asc'] = df_box['date'].dt.month
    df_box = df_box.sort_values('month_asc')

    # Draw box plots (using Seaborn)
    fig, (ax1, ax2) = plt.subplots(1, 2)
    ax1 = sns.boxplot(x='year', y='value', data=df_box, ax=ax1)
    ax1.set_xlabel('Year'), ax1.set_ylabel('Page Views')
    ax1.set_title('Year-wise Box Plot (Trend)')

    ax2 = sns.boxplot(x='month', y='value', data=df_box, ax=ax2)
    ax2.set_xlabel('Month'), ax2.set_ylabel('Page Views')
